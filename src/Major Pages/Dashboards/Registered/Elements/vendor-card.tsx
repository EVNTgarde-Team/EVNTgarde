...
export function VendorCard({
  ...
  // Function to navigate to the organizer details page
  const handleCardClick = () => {
    navigate(`/organizers/${id}`); // Navigate to the organizer details page
  };
      ...
      <Card className="group overflow-hidden dark:bg-gray-800">
        <div className="relative">
          ...
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation(); // Prevent navigating when toggling favorite
              onToggleFavorite?.();
            }}
          >
            <Star
              className={cn(
                "h-5 w-5",
                isFavorite ? "fill-yellow-400 text-yellow-400" : "text-gray-500 dark:text-gray-400"
              )}
            />
          </Button>
        </div>
        <div className="p-4">
          ...
          <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-1 text-sm">
              <span className="font-medium dark:text-white">PHP {price}</span>
              <span className="text-muted-foreground dark:text-gray-400">• {ratings} ratings</span>
            </div>
            ...
            {showHireButton && (
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation(); // Prevent navigating when clicking HIRE
                  alert(`Hiring ${name}`);
                }}
              >
                HIRE
